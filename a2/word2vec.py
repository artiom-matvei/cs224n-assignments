...
def sigmoid(x):
    ...
    s = np.divide( np.exp(x), np.exp(x) + 1)
    ### END YOUR CODE
...
def naiveSoftmaxLossAndGradient(
    centerWordVec: np.ndarray,
    outsideWordIdx: int,
    outsideVectors: np.ndarray,
    dataset
):
    ...
    ### to integer overflow.
    nbOfWords = np.shape(outsideVectors)[0]
    dotProductMatrix : np.ndarray = outsideVectors.dot(centerWordVec)
    softMaxOfDotProductMatrix = softmax(dotProductMatrix)
    loss = -1 * np.log(softMaxOfDotProductMatrix[outsideWordIdx])
    y = np.zeros(nbOfWords)
    y[outsideWordIdx] = 1
    yHat = softMaxOfDotProductMatrix
    gradCenterVec = outsideVectors.transpose().dot( (yHat - y))
