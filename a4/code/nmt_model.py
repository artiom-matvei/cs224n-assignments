...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # assert init_decoder_hidden.shape == torch.tensor([batch_size, 2*self.hidden])
        init_decoder_cell = self.c_projection(
            torch.cat((last_cell[0], last_cell[1]), 1)
        )
        # assert init_decoder_cell.shape == torch.tensor([batch_size, 2*self.hidden])
    ...
    def decode(
        ...
        # TODO: why there is a zero tensor recommendation? should it be used somewhere?
        enc_hiddens_proj = self.att_projection(enc_hiddens)
        # tgt_len = target_padded.size(0)
        Y = self.model_embeddings.target(target_padded)
        # Y = torch.zeros(tgt_len, batch_size, self.model_embeddings.embed_size)
        for Y_t in torch.split(Y, 1, 0):
            Y_t = torch.squeeze(Y_t, 0)
            Ybar_t = torch.cat((Y_t, o_prev), 1) # Y_t is (b, e) an o_prev is (b, h)
    ...
    def step(
        ...
        alpha_t = F.softmax(e_t, dim=1)  # e_t is (b, src_len)
        alpha_t = torch.unsqueeze(alpha_t, dim=1)
        a_t = torch.bmm(alpha_t, enc_hiddens)
        a_t = torch.squeeze(a_t, dim=1)
        U_t = torch.cat((dec_hidden, a_t), dim=1) # dec_hidden is (b, h) nad a_t is (b, 2h)
