...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        # 1. for each feature, select the correct embedding in self.embedding
        x = torch.randn(w.shape[0], w.shape[1], self.embed_size)
        for i, parse in enumerate(w):
            x[i] = torch.index_select(self.embeddings, 0, w[i])
            # for j, feature in enumerate(parse):
            #     x[i][j] = self.embeddings[feature]
